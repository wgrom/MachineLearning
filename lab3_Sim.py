...
def classifyBayes(X, prior, mu, sigma):
    ...
    for k in range(Nclasses):
        for a in range(Npts):
            ...
            param2 = -0.5*np.dot(np.dot((X[a]-mu[k]),np.diag(1/np.diag(sigma[k]))),np.transpose(X[a]-mu[k])) #np.diag(1/np.diag(sigma[k])) is the inverse without inf
            param3 = math.log(prior[k],math.e)
            logProb[k,a] = param1 + param2 + param3
